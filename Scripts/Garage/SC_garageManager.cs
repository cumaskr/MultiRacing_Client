...
public class SC_garageManager : MonoBehaviour
{
    //싱글톤 정보매니져
    ...
    //차모델 말고 바퀴는 장착한것이 그대로 유지해야한다. 처음에 로드할때 가져오는 바퀴 이름을 알아야해서 저장해 둔다.
    string _carwheelAssetName;
    ...
    void Start()
    {
        ...
        StartCoroutine(this.S2C_CarList_All());
    }
    ...
    {
        Destroy(m_rotateCarModel);

        //경로를 가져온다.
        string _carbuttonname = _select_carbutton.gameObject.name;

        //스탯 받아와서 출력해주어야한다.
        StartCoroutine(S2C_Stat(_select_carbutton.name, _carwheelAssetName, _carwingAssetName));
        ...
        m_rotateCarModel = Instantiate(Resources.Load("Car/" + _carbuttonname), new Vector3(0, 0, 0), Quaternion.identity) as GameObject;
        ...
        {
            _wheel.material = Resources.Load<Material>("Wheel/" + _carwheelAssetName);
        }
        ...
        //상단에 보일 차량 로고
        m_carname_Object.transform.GetChild(0).GetComponent<Image>().sprite = Resources.Load<Sprite>("Images/Car/" + _carbuttonname);
        //상단에 보일 차량 이름
    ...
    IEnumerator S2C_CarList_All()
    {
        //HTTP통신을 할 대상 주소
        string url = "49.247.131.35/S2C_CarList_All.php";
        ...
        else
        {
            ...
            //==================================================================================차
            JArray _cars = (JArray)jobject["m_car"];

            _carwheelAssetName = (string)jobject["m_wheel"];
            ...
            if (_cars != null)
            {
                ...
                for (int i = 0; i < _cars.Count; i++)
                {
                    ...
                    _CarButton.gameObject.name = (string)_cars[i]["m_assetname"];

                    //차 버튼 1번째는 선택됬을때 표시할 이미지 이다. 
                    //ChangeToSelectCarButton()에서 사용중

                    //차버튼 2번재 텍스트창에 차이름을 표시해준다.
                    _CarButton.transform.GetChild(2).GetComponent<Text>().text = (string)_cars[i]["m_name"];

                    //차버튼 3번재 텍스트창에 차구매여부를 표시해준다. 이것을 기반으로 차 구매하기 버튼이 뜬다.
                    _CarButton.transform.GetChild(3).GetComponent<Text>().text = (string)_cars[i]["m_buy"];

                    //구매한 차라면
                    if (_CarButton.transform.GetChild(3).GetComponent<Text>().text == "1")
                    {
                        //차버튼 밑 0번째는 로고를 넣어준다.
                        _CarButton.transform.GetChild(0).GetComponent<Image>().sprite = Resources.Load<Sprite>("Images/Car/" + (string)_cars[i]["m_assetname"]);
                    }
                    //구매한 차가 아니면
                    else
                    {
                    ...
                    _CarButton.transform.GetChild(4).GetComponent<Text>().text = (string)_cars[i]["m_price"];
                    ...
                    if ((int)_cars[i]["m_equip"] == 1)
                    {
                        ...
                        m_rotateCarModel = Instantiate(Resources.Load("Car/" + (string)_cars[i]["m_assetname"]), new Vector3(0, 0, 0), Quaternion.identity) as GameObject;
                        m_rotateCarModel.transform.SetParent(m_rotateObject.transform, false);
                        m_rotateCarModel.transform.localScale = new Vector3(1.6f, 1.6f, 1.6f);
                        ...
                        //화면 상단 이름 셋팅
                        m_carname_Object.transform.GetChild(0).GetComponent<Image>().sprite = Resources.Load<Sprite>("Images/Car/" + (string)_cars[i]["m_assetname"]);
                        m_carname_Object.transform.GetChild(1).GetComponent<Text>().text = (string)_cars[i]["m_name"];
                        m_carname_Object.transform.GetChild(2).GetComponent<Text>().text = (string)_cars[i]["m_price"];
                        m_carname_Object.transform.GetChild(2).gameObject.SetActive(false);
                        m_carname_Object.transform.GetChild(3).gameObject.SetActive(true);
        ...
        //스탯 받아와서 출력해주어야한다.
        StartCoroutine(S2C_Stat(m_now_carbutton.name, _carwheelAssetName, _carwingAssetName));
    ...
    //차 선택을 할때 장착중이라는 표시가 바뀌어야한다. 구매한 차들끼리에서만 장착중이 바뀌어야한다. //기존 차량끼리 변경 or 구매할때 호출된다.
    void ChangeToSelectCarButtonImage(GameObject _prev, GameObject _new)
    {
        ...
        //여기다가 InfoManager에 차 정보에 관한 데이터를 갱신한다.(멀티플레이에서 이값을 참조하여 채팅방에 차를 생성한다.)
        m_InfoManager.m_carAssetName = _new.gameObject.name;
    }
    ...
    public void BuyCar()
    {
        ...
        if (m_buy_carbutton != null)
        {
            ...
            m_buy_carbutton.transform.GetChild(0).GetComponent<Image>().sprite = Resources.Load<Sprite>("Images/Car/" + m_buy_carbutton.gameObject.name);

            m_now_carbutton = m_buy_carbutton;
        }

    }
    //서버에 사려고하는 아이템이름을 보낸다.
    ...
    //바퀴화면으로 화면전환 함수
    public void SceneChangeToWheel()
    {
        SceneManager.LoadScene("scene_garageWheel");
    }

    //날개화면으로 화면전환 함수
    public void SceneChangeToWing()
    {
        SceneManager.LoadScene("scene_garageWing");
    }

    // car
    //_select_carbutton.gameObject.name(ChangeCar())
    //m_now_carbutton.gameObject.name(ReceiveLoginCar())
    //_carwheelAssetName / _carwingAssetName
