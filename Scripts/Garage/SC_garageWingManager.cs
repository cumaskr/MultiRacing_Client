...
public class SC_garageWingManager : MonoBehaviour
{
    //싱글톤 정보매니져
    ...
    string _carcarAssetname = null;
    //차모델 말고 날개는 장착한것이 그대로 유지해야한다. 처음에 로드할때 가져오는 날개 이름을 알아야해서 저장해 둔다.
    string _carwheelAssetname = null;

    //클릭한 모델 담는다. 날개를 셋 온오프로 하니가 생각보다 꼬인다..
    string m_selectButtonName = null;
    ...
    public void ChangeCar(GameObject _select_carbutton)
    {
        ...
        //스탯 받아와서 출력해주어야한다.
        StartCoroutine(S2C_Stat(_carcarAssetname, _carwheelAssetname, _select_carbutton.name));

        m_rotateCarModel.transform.Find(m_selectButtonName).gameObject.SetActive(false);
        m_rotateCarModel.transform.Find(_select_carbutton.name).gameObject.SetActive(true);
        ...
        //==================================================================================    
        //m_rotateCarModel.transform.Find(m_prev_carbutton.name).gameObject.SetActive(false);
        //m_rotateCarModel.transform.Find(_select_carbutton.name).gameObject.SetActive(true);
        //==================================================================================

        //상단에 보일 차량 로고
        m_carname_Object.transform.GetChild(0).GetComponent<Image>().sprite = Resources.Load<Sprite>("Images/Wing/" + _carbuttonname);
        //상단에 보일 차량 이름
        ...
        m_selectButtonName = _select_carbutton.name;
    ...
    IEnumerator S2C_WingList_All()
    {
        //HTTP통신을 할 대상 주소
        string url = "49.247.131.35/S2C_WingList_All.php";
        ...
        else
        {
            ...
            //==================================================================================차
            JArray _wings = (JArray)jobject["m_wing"];
            ...
            if (_wings != null)
            {
                ...
                for (int i = 0; i < _wings.Count; i++)
                {
                    ...
                    _CarButton.gameObject.name = (string)_wings[i]["m_assetname"];

                    //차 버튼 1번째는 선택됬을때 표시할 이미지 이다. 
                    //ChangeToSelectCarButton()에서 사용중

                    //차버튼 2번재 텍스트창에 차이름을 표시해준다.
                    _CarButton.transform.GetChild(2).GetComponent<Text>().text = (string)_wings[i]["m_name"];

                    //차버튼 3번재 텍스트창에 차구매여부를 표시해준다. 이것을 기반으로 차 구매하기 버튼이 뜬다.
                    _CarButton.transform.GetChild(3).GetComponent<Text>().text = (string)_wings[i]["m_buy"];

                    //구매한 차라면
                    if (_CarButton.transform.GetChild(3).GetComponent<Text>().text == "1")
                    {
                        //차버튼 밑 0번째는 로고를 넣어준다.
                        _CarButton.transform.GetChild(0).GetComponent<Image>().sprite = Resources.Load<Sprite>("Images/Wing/" + (string)_wings[i]["m_assetname"]);
                    }
                    //구매한 차가 아니면
                    else
                    {
                    ...
                    _CarButton.transform.GetChild(4).GetComponent<Text>().text = (string)_wings[i]["m_price"];
                    ...
                    if ((int)_wings[i]["m_equip"] == 1)
                    {
                        m_now_carbutton = _CarButton;

                        m_selectButtonName = m_now_carbutton.gameObject.name;
                        ...
                        m_rotateCarModel.transform.Find((string)_wings[i]["m_assetname"]).gameObject.SetActive(true);

                        //====================================================================================================================================================

                        //화면 상단 이름 셋팅
                        m_carname_Object.transform.GetChild(0).GetComponent<Image>().sprite = Resources.Load<Sprite>("Images/Wing/" + (string)_wings[i]["m_assetname"]);
                        m_carname_Object.transform.GetChild(1).GetComponent<Text>().text = (string)_wings[i]["m_name"];
                        m_carname_Object.transform.GetChild(2).GetComponent<Text>().text = (string)_wings[i]["m_price"];
                        m_carname_Object.transform.GetChild(2).gameObject.SetActive(false);
                        m_carname_Object.transform.GetChild(3).gameObject.SetActive(true);
        ...
        //스탯 받아와서 출력해주어야한다.
        StartCoroutine(S2C_Stat(_carcarAssetname, _carwheelAssetname, m_now_carbutton.name));
    ...
    void ChangeToSelectCarButtonImage(GameObject _prev, GameObject _new)
    {
        ...
        //여기다가 InfoManager에 날개 정보에 관한 데이터를 갱신한다.(멀티플레이에서 이값을 참조하여 채팅방에 차를 생성한다.)
        m_InfoManager.m_wingAssetName = _new.gameObject.name;        
    }
    ...
    public void BuyCar()
    {
        ...
        if (m_buy_carbutton != null)
        {
            ...
            m_buy_carbutton.transform.GetChild(0).GetComponent<Image>().sprite = Resources.Load<Sprite>("Images/Wing/" + m_buy_carbutton.gameObject.name);
